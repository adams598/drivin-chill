...
const QRScanner = () => {
  ...
  const startCamera = async () => {
    try {
      ...
      if (!isSecure) {
        const errorMsg =
          "L'accès à la caméra nécessite une connexion HTTPS. Veuillez utiliser https:// ou tester en localhost.";
      ...
      if (!navigator.mediaDevices || !navigator.mediaDevices.getUserMedia) {
        const errorMsg =
          "Votre navigateur ne supporte pas l'accès à la caméra. Veuillez utiliser un navigateur moderne (Chrome, Firefox, Safari).";
        setCameraError(errorMsg);
        toast.error("Navigateur non compatible");
        return;
      }

      // Vérifier que l'élément DOM existe - il devrait toujours être présent maintenant
      let scannerElement = document.getElementById(scannerId);
      if (!scannerElement) {
        // Attendre un peu que React ait rendu l'élément
        await new Promise((resolve) => setTimeout(resolve, 200));
        scannerElement = document.getElementById(scannerId);
        if (!scannerElement) {
          const errorMsg = `L'élément de scanner avec l'ID "${scannerId}" n'a pas été trouvé dans le DOM. Veuillez rafraîchir la page.`;
          setCameraError(errorMsg);
          toast.error("Erreur d'initialisation");
          console.error(
            "Élément scanner non trouvé:",
            scannerId,
            "Éléments avec ID dans le DOM:",
      ...
      // S'assurer que l'élément est visible avant d'initialiser Html5Qrcode
      // Html5Qrcode a besoin que l'élément soit visible dans le DOM
      ...
      for (let i = 0; i < cameraConfigs.length; i++) {
        ...
        try {
          console.log(
            `Tentative de démarrage caméra avec config ${i + 1}/${
      ...
      if (!cameraStarted) {
        console.error(
          "Toutes les tentatives de démarrage de caméra ont échoué"
        );
        throw (
          lastError ||
          new Error(
            "Impossible d'accéder à la caméra après toutes les tentatives"
          )
        );
      }
    } catch (err) {
      ...
      if (
        ...
        detailedInfo =
          "Veuillez autoriser l'accès à la caméra dans les paramètres de votre navigateur et réessayez.";
      } else if (
        ...
        detailedInfo =
          "La caméra est peut-être utilisée par une autre application. Fermez les autres applications et réessayez.";
      } else if (err?.message?.includes("HTTPS")) {
        errorMessage = err.message;
      } else {
        errorMessage += `Erreur technique. `;
        detailedInfo = `Détails: ${
          err?.message || err?.toString() || "Erreur inconnue"
        }`;
      }
    ...
    <div className="space-y-6">
      <Card className="bg-gray-800 border-gray-700">
        <CardHeader>
          ...
          <CardDescription className="text-gray-300">
            Scannez ou saisissez le QR code des clients pour valider leur entrée
            au cinéma drive-in
          </CardDescription>
        </CardHeader>
        <CardContent className="space-y-6">
          ...
          <div className="space-y-4">
            ...
            {/* Container pour le scanner - toujours présent dans le DOM pour que Html5Qrcode puisse l'initialiser */}
            <div className="relative">
              <div
                id={scannerId}
                className={`w-full max-w-md mx-auto rounded-lg overflow-hidden border-2 ${
                  isScanning ? "border-blue-500" : "border-transparent"
                }`}
                style={{
                  display: isScanning ? "block" : "none",
                  minHeight: isScanning ? "auto" : "1px",
                  ...
                  <div className="flex-1">
                    ...
                    <details className="mt-2 text-xs">
                      <summary className="cursor-pointer font-semibold text-red-800 mb-1">
                        Détails techniques (pour débogage)
                      </summary>
                    ...
                    <div className="mt-3 space-y-1 text-xs bg-red-50 p-2 rounded">
                      ...
                      <ul className="list-disc list-inside space-y-1 ml-2">
                        <li>
                          ...
                          <strong>https://</strong> (ou utilisez localhost)
                        </li>
                        <li>
                          Autorisez l'accès à la caméra quand votre navigateur
                          le demande
                        </li>
                        <li>
                          Vérifiez les paramètres de permissions de votre
                          navigateur (icône cadenas dans la barre d'adresse)
                        </li>
                        <li>
                          Sur mobile : vérifiez que la caméra n'est pas utilisée
                          par une autre app
                        </li>
                        <li>Rafraîchissez la page et réessayez</li>
                        <li>
                          Ouvrez la console (F12) pour voir les détails de
                          l'erreur
                        </li>
                      </ul>
                    </div>
          ...
          <div className="space-y-4">
            <div className="space-y-2">
              <Label className="text-white">
                Saisie manuelle - QR Code ou ID de réservation
              </Label>
              <div className="flex gap-2">
                <Input
                  value={bookingId}
                      ...
                      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                        ...
                        <div>
                          ...
                          <p className="font-semibold flex items-center">
                            ...
                            {format(
                              new Date(validationResult.booking.date),
                              "PP",
                              { locale: fr }
                            )}
                          </p>
                      ...
                      {validationResult.movie && (
                        <div className="bg-blue-50 p-3 rounded-lg">
                          <p className="font-medium flex items-center text-blue-800">
                            ...
                            {validationResult.movie.title} (
                            {validationResult.movie.duration} min)
                          </p>
                        </div>
                      )}
      ...
      <Card className="bg-gray-700 border-gray-600">
        ...
        <CardContent className="text-gray-300 space-y-2">
          <p>
            • <strong>Méthode 1 (Recommandée) :</strong> Cliquez sur "Activer la
            caméra" et pointez vers le QR code du client
          </p>
          <p>
            • <strong>Méthode 2 :</strong> Saisissez manuellement l'ID de
            réservation
          </p>
          <p>
            • <strong>Étape 3 :</strong> Vérifiez les informations affichées
            (nom, date, créneau)
          </p>
          <p>
            • <strong>Étape 4 :</strong> Cliquez "Autoriser l'entrée" si tout
            est correct
          </p>
          <p>
            • <strong>Statuts :</strong> Vert = Autorisé, Rouge = Refusé, Orange
            = Déjà entré
          </p>
          <p className="text-yellow-400 mt-2">
            ⚠️ <strong>Note :</strong> La caméra nécessite l'autorisation de
            votre navigateur
          </p>
          <p className="text-yellow-400 mt-1">
            📱 <strong>Sur mobile :</strong> Autorisez l'accès à la caméra quand
            demandé, puis pointez vers le QR code
          </p>
          <p className="text-blue-400 mt-1">
            🌐 <strong>Important :</strong> L'accès caméra fonctionne uniquement
