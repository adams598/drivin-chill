...
const SimpleMovieScheduler = () => {
  ...
  const fetchTimeSlotSettings = async () => {
    try {
      ...
      console.error(
        "Erreur lors du chargement des paramètres de créneaux:",
        error
      );
    }
  };
  ...
  const loadData = async () => {
    try {
      ...
      const schedulesRes = await axios.get(`${API}/movie-schedules`, {
        headers,
      });
      ...
      toast.error(
        `Erreur de chargement: ${error.response?.data?.detail || error.message}`
      );
    } finally {
      setLoading(false);
    }
    ...
    try {
      ...
      const data = {
        ...
        poster_url: movieFormData.poster_url
          ? movieFormData.poster_url.trim()
          : "",
        trailer_url: movieFormData.trailer_url
          ? movieFormData.trailer_url.trim()
          : "",
      };
  ...
  const handleScheduleSubmit = async (e) => {
    ...
    if (
      ...
      !scheduleFormData.time_slot ||
      !scheduleFormData.entry_time ||
      !scheduleFormData.start_time
    ) {
    ...
    if (!timePattern.test(scheduleFormData.entry_time)) {
      toast.error(
        "Format d'heure d'entrée invalide. Utilisez le format 20h45 ou 20:45"
      );
      return;
    }
    if (!timePattern.test(scheduleFormData.start_time)) {
      toast.error(
        "Format d'heure de début invalide. Utilisez le format 21h00 ou 21:00"
      );
      return;
    }
    ...
    try {
      ...
      if (editingSchedule) {
        ...
        if (
          scheduleFormData.date !== editingSchedule.schedule.date.split("T")[0]
        ) {
        ...
        if (
          scheduleFormData.entry_time !==
          (editingSchedule.schedule.entry_time || "")
        ) {
        ...
        console.log(
          "📤 Envoi PUT /api/movie-schedules/" + editingSchedule.schedule.id
        );
        console.log("Données:", updateData);

        await axios.put(
          `${API}/movie-schedules/${editingSchedule.schedule.id}`,
          updateData,
          {
            headers,
            timeout: 10000,
  ...
  const deleteSchedule = async (scheduleId) => {
    if (
      !window.confirm("Voulez-vous vraiment supprimer cette programmation ?")
    ) {
      return;
    }
        ...
        <CardContent>
          {showMovieForm && (
            <form
              ...
              className="space-y-4 mb-6 p-4 bg-gray-50 rounded-lg border-2 border-green-200"
            >
              ...
              <div>
                ...
                <input
                  ...
                  onChange={(e) => {
                    ...
                    if (helpMessage && helpMessage.type === "error") {
                      toast.warning(
                        "URL Canva détectée ! Consultez les instructions ci-dessous pour obtenir l'URL directe de l'image.",
                        {
                          duration: 5000,
                        }
                      );
                ...
                />
                {(() => {
                  const helpMessage = getImageUrlHelpMessage(
                    movieFormData.poster_url
                  );
                            ...
                            {helpMessage.quickSolution && (
                              <div className="bg-white p-3 rounded border border-red-200">
                                <p className="text-xs font-bold text-red-800 mb-2">
                                  {helpMessage.quickSolution.title}
                                </p>
                                <ol className="text-xs text-red-700 space-y-2">
                                  {helpMessage.quickSolution.steps.map(
                                    (item, idx) => (
                ...
                <p className="text-xs text-gray-500 mt-1">
                  💡 Astuce : Utilisez l'URL directe de l'image (qui se termine
                  par .jpg, .png, etc.). Pour Canva, vous devez exporter l'image
                  et l'héberger sur un service comme Imgur.
                </p>
              </div>
          ...
          <div>
            <h3 className="font-bold mb-3 text-gray-800">
              📚 Films disponibles ({movies.length})
            </h3>
            {movies.length === 0 ? (
                  ...
                  >
                    <div className="flex-1">
                      ...
                      <small className="text-gray-500">
                        {movie.duration_minutes} min • {movie.genre} •{" "}
                        {movie.age_rating.replace("_", " ")}
          ...
          <form onSubmit={handleScheduleSubmit} className="space-y-4">
            ...
            <div className="bg-yellow-50 border border-yellow-200 rounded-lg p-3 flex items-start gap-2">
              ...
              <p className="text-sm text-yellow-800">
                <strong>Important :</strong> Les horaires doivent être au format
                20h45 ou 20:45
              </p>
            </div>
                  ...
                  onClick={() => {
                    setScheduleFormData({
                      ...
                      entry_time:
                        timeSlotSettings?.third_slot_entry_time || "23h15",
                      start_time:
                        timeSlotSettings?.third_slot_start_time || "23h30",
            ...
            {movies.length === 0 && (
              <p className="text-sm text-yellow-600 italic">
                ⚠️ Ajoutez d'abord un film avant de programmer
              </p>
            )}
          </form>
        </CardContent>
