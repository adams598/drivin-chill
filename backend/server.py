...
async def send_confirmation_email(booking: TicketBooking, qr_code: str, max_retries: int = 3):
    ...
    # IMPORTANT: Utiliser directement les données sauvegardées dans le booking
    ...
    try:
        ...
        if booking_db:
            # PRIORITÉ 1: Utiliser les données directement depuis le booking (les plus précises)
            entry_time = booking_db.get("entry_time") or booking.entry_time if hasattr(booking, 'entry_time') else None
            start_time = booking_db.get("start_time") or booking.start_time if hasattr(booking, 'start_time') else None
            end_time = booking_db.get("end_time") or booking.end_time if hasattr(booking, 'end_time') else None
            ...
            # Récupérer le titre du film via content_id depuis le booking
            content_id = booking_db.get("content_id") or (booking.content_id if hasattr(booking, 'content_id') else None)
            content_type = booking_db.get("content_type") or (booking.content_type if hasattr(booking, 'content_type') else "movie")
            
            logging.info(f"📧 Données depuis booking: entry_time={entry_time}, start_time={start_time}, end_time={end_time}, content_id={content_id}, content_type={content_type}")
            
            if content_id:
                # Récupérer le titre depuis la table movies ou events selon content_type
                if content_type == "movie":
                    ...
                    if event:
                        movie_title = event.get("title", "Événement à confirmer")
                        logging.info(f"📧 Événement trouvé via content_id: {movie_title}")
        
        # FALLBACK: Si les horaires ne sont pas dans le booking, chercher dans les schedules
        if not entry_time or not start_time or not end_time:
            logging.info(f"📧 Horaires manquants dans booking, recherche dans les schedules (fallback)")
            ...
            logging.info(f"📧 Schedule trouvé (fallback): {schedule is not None}, date: {booking_date_str}, time_slot: {time_slot_str}")
            
            if schedule:
                # Récupérer les heures depuis le schedule (fallback uniquement)
                entry_time = entry_time or schedule.get("entry_time")
                start_time = start_time or schedule.get("start_time")
                end_time = end_time or schedule.get("end_time")
                
                logging.info(f"📧 Horaires depuis schedule (fallback): entry={entry_time}, start={start_time}, end={end_time}")
                
                # Récupérer le film depuis le schedule si pas encore trouvé
                if movie_title == "Film à confirmer":
                    movie = await db.movies.find_one({"id": schedule.get("movie_id")})
                    if movie:
                        movie_title = movie.get("title", "Film à confirmer")
                        logging.info(f"📧 Film trouvé via schedule (fallback): {movie_title}")
        
        # FALLBACK FINAL: Si les horaires ne sont toujours pas trouvés, utiliser TimeSlotSettings
        if not entry_time or not start_time or not end_time:
...
async def create_booking(booking_data: TicketBookingCreate):
    try:
        ...
        # IMPORTANT: Prioriser les valeurs envoyées par le frontend (content_id, content_type)
        # Ne les remplacer par celles du schedule QUE si elles ne sont pas fournies
        
        # Si content_id et content_type sont fournis par le frontend, les utiliser (priorité absolue)
        if booking_data.content_id and booking_data.content_type:
            logging.info(f"✅ Utilisation du content_id et content_type depuis le frontend: content_id={booking_data.content_id}, content_type={booking_data.content_type}")
            booking_dict["content_type"] = booking_data.content_type
            booking_dict["content_id"] = booking_data.content_id
        # Sinon, utiliser ceux du schedule trouvé (fallback)
        elif content_schedule:
            logging.warning(f"⚠️ content_id/content_type non fournis par le frontend, utilisation du schedule: content_id={content_schedule.get('content_id')}, content_type={content_schedule.get('content_type', 'movie')}")
            ...
            logging.warning(f"⚠️ content_id/content_type non fournis par le frontend, utilisation du movie_schedule: movie_id={movie_schedule.get('movie_id')}")
...
async def get_booking_details_from_schedules(
    ...
    if movie_schedule:
        logging.info(f"✅ Trouvé movie_schedule: movie_id={movie_schedule.get('movie_id')}, date={movie_schedule.get('date')}, time_slot={movie_schedule.get('time_slot')}")
        # Récupérer entry_time, start_time et end_time depuis movie_schedule
        entry_time = movie_schedule.get("entry_time")
        start_time = movie_schedule.get("start_time")
        end_time = movie_schedule.get("end_time")
        
        # Récupérer le titre du film via movie_id
        ...
        if movie_id:
            ...
            if movie:
                ...
                logging.warning(f"⚠️ Film non trouvé avec movie_id={movie_id} depuis movie_schedule")
    
    # PRIORITÉ : Si content_id et content_type sont disponibles, utiliser directement pour récupérer le titre
    # (cela fonctionne même si aucun schedule n'est trouvé)
    if not movie_title and content_id and content_type:
        if content_type == "movie":
    ...
    if content_schedule:
        ...
        if not entry_time or not start_time:
            ...
            if movie_schedule_temp:
                ...
                if not end_time:
                    end_time = movie_schedule_temp.get("end_time")
                logging.info(f"✅ Horaires trouvés dans movie_schedule: entry_time={entry_time}, start_time={start_time}, end_time={end_time}")
    else:
...
async def get_admin_dashboard(admin = Depends(get_admin_user)):
    try:
        ...
        for booking in recent_bookings:
            ...
            booking_dict["start_time"] = start_time_from_booking
            booking_dict["end_time"] = end_time_from_booking
            
            # Récupérer le titre du film depuis la table movies/events en utilisant content_id
            movie_title = None
            if booking_obj.content_id and booking_obj.content_type:
                if booking_obj.content_type == "movie":
                    movie = await db.movies.find_one({"id": booking_obj.content_id, "is_active": True})
                    if not movie:
            ...
            if not movie_title:
                ...
                if not end_time_from_booking:
                    booking_dict["end_time"] = booking_details.get("end_time")
...
async def get_all_admin_bookings(admin = Depends(get_admin_user)):
    try:
        ...
        for booking in bookings:
            ...
            # Log pour déboguer
            logging.info(f"📋 Réservation {booking_obj.id} ({booking_obj.first_name} {booking_obj.last_name}) - entry_time={booking_dict.get('entry_time')}, start_time={booking_dict.get('start_time')}, end_time={booking_dict.get('end_time')}, movie_title={booking_dict.get('movie_title')}, content_id={booking_obj.content_id}, content_type={booking_obj.content_type}, date={booking_obj.booking_date}, time_slot={booking_obj.time_slot}")
            ...
            if not booking_dict.get("entry_time") or booking_dict.get("entry_time") == booking_obj.time_slot:
                logging.warning(f"⚠️ Réservation {booking_obj.id} - entry_time non trouvé ou égal au time_slot. entry_time={booking_dict.get('entry_time')}, time_slot={booking_obj.time_slot}")
